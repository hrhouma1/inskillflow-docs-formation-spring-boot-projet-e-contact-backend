...
Le navigateur et Postman sont traités différemment par Spring Security par défaut.

**Pour le navigateur :**
- Spring Security détecte que c'est un client web (via les headers Accept)
- Il utilise le mécanisme "form login" qui redirige vers une page de connexion
- C'est pratique pour les applications web classiques

**Pour Postman/curl :**
- Spring Security détecte que c'est un client API
- Il renvoie directement un statut HTTP 401 Unauthorized
- Pas de redirection, juste une réponse HTTP brute
...
Si ton controller n'est jamais appelé, c'est très probablement parce que **Spring Security a bloqué la requête avant qu'elle n'atteigne le controller**.

La requête passe par la chaîne de filtres Security AVANT d'arriver au controller. Si l'un de ces filtres décide que la requête n'est pas autorisée (pas de token, mauvais rôle, etc.), il renvoie une erreur 401/403 et le controller n'est jamais exécuté.
